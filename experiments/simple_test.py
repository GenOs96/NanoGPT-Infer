...
def get_artifacts_dir() -> Path:
    artifacts_dir = Path(__file__).resolve().parent / "artifacts" / "simple_test_profiler"
...
def save_summary_csv(row: dict[str, float | int | str], profile_tool: str, artifacts_dir: Path) -> Path:
    csv_path = artifacts_dir / f"simple_test_profiler_summary_{profile_tool}.csv"
...
def print_nsight_commands(args: argparse.Namespace) -> None:
    ...
    nsys_cmd = (
        ...
        "--sample=none "
        "--output experiments/artifacts/simple_test_profiler/nsys_simple_test_timeline "
        + base_cmd
    )
    ncu_cmd = (
        "ncu --set full "
        ...
        "--export experiments/artifacts/simple_test_profiler/ncu_simple_test_decode "
        + base_cmd
    )
