...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "End-to-end KV-cache profiling harness for PyTorch Profiler, "
    ...
    parser.add_argument(
        "--mark-decode-steps",
        action="store_true",
        help="Emit per-step NVTX ranges inside the decode loop.",
    )
    parser.add_argument(
        "--disable-nvtx",
        action="store_true",
...
def get_artifacts_dir() -> Path:
    artifacts_dir = Path(__file__).resolve().parent / "artifacts" / "kv_cache_profiler"
...
def build_model(model_name: str, device: str) -> GPT:
    ...
    model = model.to(device=device, dtype=get_dtype())
    model.eval()
    return model
...
def run_timed_steps(
    ...
    return step_latencies[warmup_steps:], peak_memories[warmup_steps:]
...
def run_torch_profile_config(
    ...
    return {
        ...
        "max_new_tokens": max_new_tokens,
        "total_context_len": prompt_length + max_new_tokens,
...
def save_summary_csv(rows: list[dict[str, float | int | str]], profile_tool: str, artifacts_dir: Path) -> Path:
    csv_path = artifacts_dir / f"kv_cache_profiler_summary_{profile_tool}.csv"
...
def main() -> None:
    ...
    summary_csv = save_summary_csv(rows, args.profile_tool, artifacts_dir)
